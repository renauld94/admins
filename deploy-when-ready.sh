...
# Learning Odyssey Deployment - Run When Connection is Available
# This script will deploy via proxy jump: root@136.243.155.166 -> simonadmin@10.0.0.104
...
echo "🚀 Learning Odyssey Deployment Script"
echo "====================================="
...
if ssh -o ConnectTimeout=10 -o BatchMode=yes -J $PROXY_HOST $TARGET_HOST "echo 'Connection successful'" 2>/dev/null; then
    print_success "Connection established! Proceeding with deployment..."
else
    print_error "Connection failed. Please ensure:"
    echo "1. SSH keys are configured for root@136.243.155.166"
    echo "2. Proxy server is accessible"
    echo "3. Target server is reachable from proxy"
    echo "4. Network connectivity is working"
    echo ""
    print_status "You can also use the manual deployment package:"
    echo "1. scp learning-odyssey-deployment.tar.gz $TARGET_HOST:~/"
    echo "2. ssh -J $PROXY_HOST $TARGET_HOST"
