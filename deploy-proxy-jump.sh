...
# Learning Odyssey Deployment via Proxy Jump
# Deploy to Moodle course via proxy jump: root@136.243.155.166 -> simonadmin@10.0.0.104
...
echo "🚀 Starting Learning Odyssey Deployment via Proxy Jump..."
...
if ! ssh -o ConnectTimeout=10 -o BatchMode=yes $PROXY_HOST "echo 'Proxy connection successful'" 2>/dev/null; then
    print_error "Cannot connect to proxy $PROXY_HOST"
    print_status "Please ensure:"
    echo "1. SSH key is properly configured for root@136.243.155.166"
...
if ! ssh -J $PROXY_HOST -o ConnectTimeout=10 -o BatchMode=yes $TARGET_HOST "echo 'Target connection successful'" 2>/dev/null; then
    print_error "Cannot connect to target $TARGET_HOST via proxy"
    print_status "Please ensure:"
    echo "1. Proxy can reach target server"
    echo "2. Target server is accessible from proxy"
...
print_status "Creating remote directory structure via proxy..."
ssh -J $PROXY_HOST $TARGET_HOST "mkdir -p $ODYSSEY_PATH/{js,css,images,ai,analytics}"

# Copy files to remote server via proxy
print_status "Copying implementation files via proxy..."
...
print_success "Files copied successfully via proxy!"

# Create Moodle integration files on remote server via proxy
print_status "Creating Moodle integration files via proxy..."
...
print_status "Setting file permissions via proxy..."
...
# Create Moodle integration instructions
ssh -J $PROXY_HOST $TARGET_HOST "cat > $ODYSSEY_PATH/MOODLE_INTEGRATION.md << 'EOF'
...
print_success "Moodle integration files created via proxy!"

# Test deployment
print_status "Testing deployment via proxy..."
ssh -J $PROXY_HOST $TARGET_HOST "cd $ODYSSEY_PATH && ./test-deployment.sh"

print_success "🎉 Learning Odyssey deployment completed successfully via proxy jump!"
...
echo "1. SSH to server: ssh -J $PROXY_HOST $TARGET_HOST"
echo "2. Add odyssey-course.html as a page resource in Moodle"
echo "3. Configure AI Oracle and Databricks services"
