...
# Uses Proxmox server (136.243.155.166) as jump host to reach CT 150 (10.0.0.150)
...
if ! ssh -o ConnectTimeout=10 -o ProxyJump=$PROXY_HOST $TARGET_USER@$TARGET_HOST "echo 'Connection successful'" > /dev/null 2>&1; then
...
ssh -o ProxyJump=$PROXY_HOST $TARGET_USER@$TARGET_HOST "mkdir -p /var/backups/portfolio && cp -r $TARGET_PATH/* /var/backups/portfolio/$(date +%Y%m%d_%H%M%S)/ 2>/dev/null || true"

# Upload files to target server via proxy
echo "⬆️ Uploading portfolio files to target server via proxy..."
rsync -avz --progress --delete \
    -e "ssh -o ProxyJump=$PROXY_HOST" \
    --exclude='.git' \
...
ssh -o ProxyJump=$PROXY_HOST $TARGET_USER@$TARGET_HOST "chown -R www-data:www-data $TARGET_PATH && chmod -R 755 $TARGET_PATH"

# Restart web server
echo "🔄 Restarting web services..."
ssh -o ProxyJump=$PROXY_HOST $TARGET_USER@$TARGET_HOST "systemctl reload nginx || systemctl restart apache2 || true"
...
echo "   ssh -o ProxyJump=$PROXY_HOST $TARGET_USER@$TARGET_HOST"
echo "   rsync -e \"ssh -o ProxyJump=$PROXY_HOST\" ..."
