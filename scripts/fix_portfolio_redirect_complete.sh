#!/bin/bash
echo "🔧 COMPLETE PORTFOLIO REDIRECT FIX"
echo "=================================="
echo "Issue: www.simondatalab.de redirecting to Moodle"
echo "Root Cause: Moodle installed in /var/www/html instead of subdirectory"
echo "Solution: Move Moodle to subdirectory, deploy portfolio to root"
echo ""
...
mkdir -p /var/backups/moodle-move-$(date +%Y%m%d-%H%M%S)
...
cat > /etc/apache2/sites-available/moodle.simondatalab.de.conf << 'EOF'
